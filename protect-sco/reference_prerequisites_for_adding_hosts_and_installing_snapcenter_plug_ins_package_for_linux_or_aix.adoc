...
---
= Requisitos previos para añadir hosts e instalar el paquete de complementos SnapCenter para Linux o AIX
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Debes haber instalado Java 17 (para SnapCenter 6.2.1 y posteriores) o Java 11 (para SnapCenter 6.2 y anteriores) en tu host Linux o AIX.
+
** Java de Oracle, OpenJDK o SAP JDK es compatible con Linux.
** IBM Java para AIX. Puedes descargarlo de https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["Descargas de IBM Semeru Runtimes"^].
+

NOTE: Asegúrate de haber instalado únicamente la edición certificada de Java 17 (para SnapCenter 6.2.1 y posteriores) o Java 11 (para SnapCenter 6.2 y anteriores) en el host Linux.
...
Paquetes de software obligatorios
 a| 
* Java 17 (Oracle Java, OpenJDK o SAP JDK) para SnapCenter 6.2.1 y posteriores o Java 11 para SnapCenter 6.2 y anteriores



NOTE: Asegúrate de haber instalado únicamente la edición certificada de Java 17 para SnapCenter 6.2.1 y posteriores o Java 11 para SnapCenter 6.2 y anteriores en el host Linux.

Si has actualizado Java a la última versión, debes asegurarte de que la opción JAVA_HOME ubicada en /var/opt/snapcenter/spl/etc/spl.properties esté configurada con la versión correcta de Java y la ruta correcta.

|===
Para obtener la información más reciente sobre las versiones compatibles, consulte la https://imt.netapp.com/imt/imt.jsp?components=180320;180338;&solution=1257&isHWU&src=IMT["Herramienta de matriz de interoperabilidad de NetApp"] .
...
* Edita el archivo `/etc/ssh/sshd_config` para configurar los algoritmos de código de autenticación de mensajes: MACs hmac-sha2-256 y MACs hmac-sha2-512.
+
...
Paquetes de software obligatorios
 a| 
* IBM Java 17 para SnapCenter 6.2.1 y versiones posteriores o IBM Java 11 para SnapCenter 6.2 y versiones anteriores
...
CAUTION: Aunque se te permite crear credenciales después de desplegar hosts e instalar plug-ins, la *mejor práctica* es crear credenciales después de haber añadido SVMs y antes de desplegar hosts e instalar plug-ins.
...
NOTE: Si especificas detalles incorrectos al crear una credencial, aparecerá un mensaje de error. Debes seleccionar *Cancelar* y luego volver a intentarlo.
...
. Selecciona image:../media/filter_icon.gif["icono de filtro"] y luego selecciona el nombre de host y el tipo de base de datos para filtrar los recursos.
+
A continuación, puedes seleccionar image:../media/filter_icon.gif["icono de filtro"] para cerrar el panel de filtrado.

. Selecciona la base de datos y luego selecciona *Database Settings* > *Configure Database*.
...
+
Para más información, consulta link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["Nodos preferidos en la configuración de RAC"^].
