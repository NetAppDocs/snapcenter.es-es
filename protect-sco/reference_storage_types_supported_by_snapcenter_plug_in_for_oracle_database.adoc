...
---
= Tipos de almacenamiento compatibles con el complemento SnapCenter para Oracle Database
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Otros hipervisores
 a| 
Se admite la ejecución de Linux bajo KVM, Xen, Hyper-V e hipervisores hyperscaler cuando el sistema operativo invitado tiene acceso directo TCP/IP a los recursos de almacenamiento ONTAP necesarios usando los siguientes métodos:
...
NOTE: Los datastores y las presentaciones de dispositivos sin procesar no son compatibles fuera de VMware.
