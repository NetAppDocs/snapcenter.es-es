...
Para obtener la información más reciente sobre las versiones de Oracle admitidas, consulte https://["Herramienta de matriz de interoperabilidad de NetApp"^].
...
La información relativa a los parámetros que se pueden utilizar con el comando y sus descripciones se puede obtener ejecutando Get-Help nombre_comando. Como alternativa, también puede consultar la https://["Guía de referencia de comandos del software SnapCenter"^].
