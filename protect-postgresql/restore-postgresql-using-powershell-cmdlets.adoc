...
summary: La restauración de una copia de seguridad PostgreSQL incluye iniciar una sesión de conexión con el servidor SnapCenter, mostrar una lista de backups y recuperar información de los backups, y restaurar una copia de seguridad. 
---
...
NOTE: AppObjectId es «Host\Plugin\UID», donde UID = <instance_name> es para un recurso de instancia de PostgreSQL detectado manualmente y UID = <instance_name>\<databse_name> es para un recurso de clúster PostgreSQL. Puede obtener el ResourceID a partir del cmdlet Get-smResources.
