...
El disco compartido no se crea mediante el plugin para Windows. Debe crear y luego agregar el LUN compartido a cada máquina virtual del clúster. Para obtener más información, consulte https://["Equipos virtuales en clúster entre hosts físicos"^].
