...
summary: Antes de añadir un host e instalar los paquetes de plugins, debe cumplir con todos los requisitos. El plugin SnapCenter para IBM DB2 está disponible en entornos Windows y Linux. 
---
= Requisitos previos para añadir hosts e instalar el plugin de SnapCenter para IBM DB2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Antes de añadir un host e instalar los paquetes de plugins, debe cumplir con todos los requisitos. El plugin SnapCenter para IBM DB2 está disponible en entornos Windows y Linux.
...
* Debe haber habilitado la conexión SSH por contraseña para el usuario raíz o no raíz.
* Debe haber instalado la biblioteca *mksh* en su host Linux.
...
Para ejecutar un comando complementario en el plugin de SnapCenter para IBM DB2, debe incluirlo en el `allowed_commands.config` archivo.

`allowed_commands.config` El archivo está ubicado en el subdirectorio «ETC» del directorio del plugin de SnapCenter para IBM DB2.
...
Ruta personalizada: `<custom_Directory>/NetApp/snapcenter/scc/etc/allowed_commands.config`
