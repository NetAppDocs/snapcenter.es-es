...
---
= Clonar a partir de una copia de seguridad de una base de datos de SQL Server usando SnapCenter
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Para EMS, puedes consultar https://docs.netapp.com/us-en/snapcenter/admin/concept_manage_ems_data_collection.html["Gestiona la recopilación de datos EMS en SnapCenter"]

. Revisa el resumen y luego selecciona *Finalizar*.
. Supervise el progreso de la operación seleccionando *Monitor* > *Trabajos*.
+

NOTE: Después de crear el clon, no debe cambiar nunca el nombre.
